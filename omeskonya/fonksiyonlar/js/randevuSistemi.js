		...
		{		randevuTarihi=document.getElementById("tarih").value;
			if(a!=null) //herhangi bir saat(radio) seçilmişse
			{
				...
				'saat': randevuSaati     
					...
					$.ajax({
                    url:'fonksiyonlar/php/randevuKaydet.php',
                    type:'POST',
                    data:values,
					dataType:'json',
				  ...
				  beforeSend: function() {
				   ...
				   error:function(ma,ydin)
					...
					$('.modal-body').html("Lütfen Bir randevu saati seçiniz!");														
					$("#myBtn").click();
...
function randevuIptal(randevuId,tc,grpid,randevuTarihi,randevuSaati)
{
	...
	if(durum)
	{
		var values = {
				...
				$.ajax({
                    url:'fonksiyonlar/php/randevuIptal.php',
                    type: 'POST',
                    data:values,
					dataType:'json',
			        success:function(data){
