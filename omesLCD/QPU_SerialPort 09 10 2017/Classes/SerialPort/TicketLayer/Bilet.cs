...
namespace QPU_SerialPort.Classes.TicketLayer
{
    public partial class Bilet
    {
        ...
        public void Create()
        {
            ...
            if (TicketNumber != 0)
            {
                ...
                {
                    Thread.Sleep(100);
                    QPU_SerialPort.Program.Communice.TicketResponse(
                        newBilet.BiletNo.ToString(),
                        (newBilet.ButonID - 1).ToString(),
                        newBilet.BiletMakineAdresi.ToString()
                        );
                    Thread.Sleep(100);
                    QPU_SerialPort.Program.Communice.MoreThanOneTicket(
                        WaitingPersonCount,
                        bmButon.BiletKopyaSayisi.ToString(),
                        newBilet.BiletMakineAdresi.ToString()
                        );
        ...
        public int GetNextTicketNumber()
        {
            ...
            string strWhereSQL = string.Format("Where GRPID={0} AND (SIS_TAR BETWEEN '{1}' AND '{2}') AND (OZEL_MUSTERI='False') AND (TRANSFER='False')",
               newBilet.GrupID,
   ...
   "TOP (1) BILET_NO")["DataTable"];
        ...
        private static bool CheckAvailableTicket()
        {
            if (!bmButon.Aktif)
            {
                Program.Communice.HasNotTicketResponse(newBilet.ButonID.ToString(),
                    newBilet.BiletMakineAdresi.ToString());
                return false;
            }
